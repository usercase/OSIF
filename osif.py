...
def dump_mail():
	...
	try:
		...
		out = open('output/' + n[0].split(' ')[0] + '_emails_' + today + '.txt','w')

		for i in a['data']:
			x = requests.get("https://graph.facebook.com/"+i['id']+"?access_token="+token)
                        z = json.loads(x.text)
			print('Wait a moment, this may take few minutes depending on your number of friends')
			try:
				out.write(G + z['name'] + W + R + ';' + W + z['email'] + '\n')
		...
		print '[*] file saved : output/' + n[0].split(' ')[0] + '_emails_' + today + '.txt'
		main()
